...
export type SalesNote = {
  tag?: string;          // optional label like "spectrum", "objection", etc.
  region?: string | null;
  title: string;
  body: string;
};
...
export const pitch =
  salesNotes.find(n => n.title.toLowerCase().includes("door pitch"))?.body ??
  salesNotes[0]?.body ?? "";

export const playbook = salesNotes.filter(
  n => !n.title.toLowerCase().includes("door pitch")
);

// ---- OPTIONAL: one object export if you prefer a single import ----
export const salesContent = { pitch, playbook, notes: salesNotes };
